...
Notes:
    ...
    * IMPORTANT: Check the path to Java JDK and Python
        set JDK_PATH below
        set PY3_PATH and/or PY2_PATH below
    * Expects data sets to be in a local subdirectory called `student_datasets`
    * Works with .java, .js, .py, .py2, .py3, .c, .cpp file types
"""

__version__ = "0.6.2021.03.03.1"
...
def verify_paths():
    """
    Verify that the paths for Java, Python, and RingoJS are valid
    ...
    if PY2_PATH and not os.path.isfile(PY2_PATH):
        display_error("PY2_PATH was set to `{}` which is NOT a file (python executable)".format(PY2_PATH))
        valid = False
    ...
    if RINGO_JS_PATH and (not os.path.isfile(RINGO_JS_PATH) or os.path.basename(RINGO_JS_PATH) != "run.jar"):
        display_error("RINGO_JS_PATH was set to {} which is NOT a file (RingoJS run.jar)".format(RINGO_JS_PATH))
        valid = False
...
def determine_problem_number(solution_file_path):
    ...
    pattern = r"prob(\d{2}).(java|py|py2|py3|c|cpp|js)$"
...
def pre_process_source(problem_number, file_type, filename):
    ...
    elif file_type in ["py", "py2", "py3"]:
        # Default to Python 3
        python_path = PY3_PATH if file_type != "py2" else PY2_PATH
        run_command = [python_path, filename]
    elif file_type == "js":
        run_command = [java_command, "-cp", ".", "-jar", RINGO_JS_PATH, filename]
    elif file_type in ["c", "cpp"]:
        # Select the correct compiler
        compiler = "gcc" if file_type == "c" else "g++"
        compile_command = [compiler, "-lm", "-o", problem_exe_name, filename]
        # Only build the run command if the compile was a success
